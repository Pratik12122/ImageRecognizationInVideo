...
public class DecodeAndCaptureFrames
{
  ...
  private static void processFrame(IVideoPicture picture, BufferedImage image, Set<Long> imageFramesSet)
  {
    try
    {
      ...
      if (picture.getPts() - mLastPtsWrite >= NANO_SECONDS_BETWEEN_FRAMES)
      {
        ...
        // write out PNG
        Long seconds = ((int)picture.getPts()) / Global.DEFAULT_PTS_PER_SECOND;
        if(imageFramesSet.contains(seconds)) {
        	String filename = "frame_"+seconds+".png"; 
        	File file = new File("c://CapturedFrames//"+filename);
  ...
  public static void processFrames(Set<Long> timestamps)
  {
//    if (args.length <= 0)
//      throw new IllegalArgumentException(
//        "must pass in a filename as the first argument");

    String filename = "C:\\Users\\Ness167279\\Downloads\\office.mp4";
      ...
      if (packet.getStreamIndex() == videoStreamId)
      {
        ...
        while(offset < packet.getSize())
        {
          ...
          if (picture.isComplete())
          {
            ...
            processFrame(newPic, javaImage,timestamps);
          }
        }
      }
      else
